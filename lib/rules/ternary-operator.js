  ...
  create(context) {
    ...
    return {
      JSXExpressionContainer(node) {
        //console.log('ternary-operator.js JSXExpressContainer: node =', node);
        const {expression} = node;
        if (expression.type !== 'ConditionalExpression') return;

        const formattedTernary = getConditionalExpressionText(expression);
        //console.log('ternary-operator.js JSXExpressContainer:',
        //  'formattedTernary =', formattedTernary);
        const text = `{\n${formattedTernary}\n}`;
      ...
      VariableDeclaration(node) {
        //console.log('ternary-operator.js VariableDeclartion: node =', node);
